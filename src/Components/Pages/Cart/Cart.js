...
function Cart() {
  ...
  return (
    <div className='container mt-5'>
            ...
            carts.map((item) => {
              return (
                <tr key={item.id}>
                  ...
                  <td>{item.product.prices}.000</td>
                  <td>{item.quantity}</td>
                  <td>{item.quantity * item.product.prices}.000</td>
                  <td><MdDelete/></td>
                </tr>
              )
            })
      ...
      <h4 className='font-weight-bold mb-4 d-flex'>Tổng tiền : <h4 className='ml-5 text-success'>{totalPrice}.000đ</h4></h4>
    </div>
  )
}
